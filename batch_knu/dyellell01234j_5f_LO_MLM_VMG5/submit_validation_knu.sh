...
echo "run_template $tag"
echo '#!/bin/bash' > $scriptname                           
echo 'SECTION=`printf %03d $1`' >> $scriptname
echo "WORKDIR=`pwd`">> $scriptname
echo 'cd $WORKDIR' >> $scriptname
#echo 'echo "#### Extracting cmssw ####"'>> $scriptname
#echo 'tar -zxvf INPUT.tar.gz'>> $scriptname
echo 'echo "#### cmsenv ####"'>> $scriptname
echo 'export CMS_PATH=/cvmfs/cms.cern.ch'>> $scriptname
echo 'source $CMS_PATH/cmsset_default.sh'>> $scriptname
echo 'export SCRAM_ARCH=slc6_amd64_gcc630'>> $scriptname

echo 'pushd CMSSW_9_3_8/src'>> $scriptname
echo 'scram build ProjectRename'>> $scriptname
echo 'eval `scramv1 runtime -sh`'>> $scriptname
echo 'popd'>> $scriptname
echo "mkdir -p tmp_$tag" >> $scriptname
echo "cp $python_cfg tmp_$tag/" >> $scriptname
echo "pushd tmp_$tag" >> $scriptname
echo "cmsRun $python_cfg &> log_${tag}.txt">> $scriptname
#echo 'mkdir -p outputs' >> $scriptname
echo "mkdir -p OUTPUT" >> $scriptname 
echo "mv *inDQM.root $WORKDIR/OUTPUT/OUTPUT_${tag}.root">> $scriptname
echo "mv log_${tag}.txt $WORKDIR/OUTPUT/" >> $scriptname
echo "popd" >> $scriptname
echo "rm -rf tmp_$tag"  >> $scriptname
...
#    cp run_template.sh run_$JOB.sh
    JOBDIR=`pwd`
    for IJOB in `seq 0 ${NJOB}`; do

	run_template run_${JOB}_${IJOB}.sh $PYTHON $IJOB
#    find . -name run_$JOB.sh | xargs perl -pi -e s/__SCRIPT__/$PYTHON/g   
	#pwd
	qsub -q long -o $JOBDIR -e $JOBDIR -N ${JOB}_${IJOB} -l walltime=672:00:00,cput=672:00:00 run_${JOB}_${IJOB}.sh 
    #qsub -q long -o $PWD -e $PWD -N aa aa.sh
...
OTAGLIST+=( dyellell01234j_5f_LO_MLM_VMG5_261_false_pdfwgt dyellell01234j_5f_LO_MLM_VMG5_261_true_pdfwgt dyellell01234j_5f_LO_MLM_VMG5_26x_false_pdfwgt dyellell01234j_5f_LO_MLM_VMG5_26x_true_pdfwgt )
...
GRIDPACKLIST+=( dyellell01234j_5f_LO_MLM_VMG5_261_false_pdfwgt_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz dyellell01234j_5f_LO_MLM_VMG5_261_true_pdfwgt_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz dyellell01234j_5f_LO_MLM_VMG5_26x_false_pdfwgt_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz dyellell01234j_5f_LO_MLM_VMG5_26x_true_pdfwgt_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz)
...
#    run_template
    ...
    submit_batch $OTAG $NJOBS cmsrun_$OTAG.py $GRIDPACK
